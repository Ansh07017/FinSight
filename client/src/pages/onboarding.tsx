    ...
    <div className="min-h-screen w-full flex items-center justify-center bg-background p-4">
      <div className="w-full max-w-lg space-y-8">
        <div className="text-center space-y-2">
          <img src={logoImg} alt="Logo" className="w-12 h-12 mx-auto" />
          <h1 className="text-3xl font-bold text-white">Let's set up your profile</h1>
          <p className="text-muted-foreground">Tell us a bit about your financial situation</p>
        </div>
          ...
          <CardHeader>
            <div className="flex items-center justify-between mb-4">
              <div className="flex space-x-2">
                <div className={`h-2 w-12 rounded-full ${step >= 1 ? 'bg-primary' : 'bg-secondary'}`} />
                <div className={`h-2 w-12 rounded-full ${step >= 2 ? 'bg-primary' : 'bg-secondary'}`} />
              </div>
              <span className="text-sm text-muted-foreground">Step {step} of 2</span>
            </div>
            <CardTitle className="text-2xl text-white">
              {step === 1 ? "What describes you best?" : "Financial Snapshot"}
            </CardTitle>
            <CardDescription>
              {step === 1 
                ? "This helps us tailor the experience for you." 
                : "Enter your current numbers to start tracking accurately."}
            </CardDescription>
          </CardHeader>
            ...
            {step === 1 ? (
              <RadioGroup defaultValue="salaried" onValueChange={(v) => setUserType(v as any)} className="grid grid-cols-1 gap-4">
                <div>
                  ...
                  <Label
                    htmlFor="salaried"
                    className="flex items-center justify-between rounded-md border-2 border-muted bg-popover p-4 hover:bg-accent hover:text-accent-foreground peer-data-[state=checked]:border-primary peer-data-[state=checked]:bg-primary/10 cursor-pointer transition-all"
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <div className="font-semibold text-white text-lg">Salaried Professional</div>
                        <div className="text-sm text-muted-foreground">I receive a fixed monthly income</div>
                      </div>
                    </div>
                    <Check className="w-5 h-5 text-primary opacity-0 peer-data-[state=checked]:opacity-100" />
                  </Label>
                </div>
                ...
                <div>
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <div className="font-semibold text-white text-lg">Student / Unemployed</div>
                        <div className="text-sm text-muted-foreground">My income varies (pocket money, etc.)</div>
                      </div>
                    </div>
                    <Check className="w-5 h-5 text-primary opacity-0 peer-data-[state=checked]:opacity-100" />
                  </Label>
                ...
                {userType === "salaried" && (
                  <>
                    ...
                    <div className="bg-primary/10 p-3 rounded-md text-sm text-primary border border-primary/20">
                      <p>⚡ Your balance will automatically refresh on day {salaryDate || "1"} of every month.</p>
                    </div>
                  </>
                )}

                {userType === "unemployed" && (
                  <div className="bg-secondary/30 p-3 rounded-md text-sm text-muted-foreground border border-border">
                    <p>💡 You can add income manually whenever you receive money (e.g., from Papa, Pocket Money).</p>
                  </div>
                )}
              </div>
            )}
