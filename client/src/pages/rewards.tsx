...
import { behavioral, profile, settings } from "@/lib/api";
import { Trophy, Star, Target, TrendingUp, Lock, Gift, Zap, ShieldCheck, Loader2 } from "lucide-react";
...
// Currency Symbol Helper (Restored and used to remove hardcoded symbols)
...
export default function RewardsPage() {
  // 1. MODULAR FETCH: Parallel queries for maximum speed
  const { data: behavioralSummary, isLoading: summaryLoading } = useQuery({
  ...
  // Show global loader only for the core identity paint
  if (profileLoading && historyLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] space-y-4">
        <Loader2 className="w-10 h-10 animate-spin text-[#00D6AB]" />
        <p className="text-muted-foreground animate-pulse">Calculating your rewards status...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="md:col-span-2 bg-linear-to-br from-card to-secondary/30 border-border/50 relative overflow-hidden">
        ...
        <Card className="bg-card border-border/50 flex flex-col justify-center items-center text-center p-6 border-t-4 border-t-[#00D6AB]">
          ...
          <p className="text-4xl font-black text-[#00D6AB] mb-2">{profileResponse?.profile?.rewardPoints || 0}</p>
      ...
      {/* Ways to Earn - UPDATED: Removed hardcoded currency */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card className="bg-card border-border/50">
          ...
          <CardContent>
            <div className="space-y-4">
              {[
                { title: "B-SAVE Logs", desc: `Every ${currencySymbol}1 saved through behavioral choices`, points: "1 XP" },
            ...
            <div className="p-4 rounded-lg bg-[#00D6AB]/5 border border-[#00D6AB]/20">
               <div className="flex items-start gap-3">
                 ...
                 <p className="text-xs text-muted-foreground leading-relaxed">
                   <strong className="text-white">Pro Tip:</strong> Users who log "Transport" behavioral wins save an average of {currencySymbol}2,400 more per month.
                 </p>
               </div>
            </div>
          </CardContent>
